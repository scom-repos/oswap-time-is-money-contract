...
import Bin from "./TimeIsMoneyEtherV2.json";
...
export class TimeIsMoneyEtherV2 extends _Contract{
    ...
    private assign(){
        ...
        let lock_send = async (to:string, options?: number|BigNumber|TransactionOptions): Promise<TransactionReceipt> => {
            let result = await this.send('lock',[to],options);
            return result;
        }
        let lock_call = async (to:string, options?: number|BigNumber|TransactionOptions): Promise<void> => {
            let result = await this.call('lock',[to],options);
            return;
        }
        let lock_txData = async (to:string, options?: number|BigNumber|TransactionOptions): Promise<string> => {
