...
export class TimeIsMoneyEther extends _Contract{
    ...
    private assign(){
        ...
        let lock_send = async (options?: number|BigNumber|TransactionOptions): Promise<TransactionReceipt> => {
            let result = await this.send('lock',[],options);
            return result;
        }
        let lock_call = async (options?: number|BigNumber|TransactionOptions): Promise<void> => {
